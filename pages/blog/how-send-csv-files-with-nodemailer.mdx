...
Note: That `--y` flag is for people that don't have much time 😛.
...
`json2csv` modules is used to convert JSON objects to CSV format in a easy and neat manner.
...
You should now receive an email on the receiver mailbox with the an attached CSV file with the appropriate data! Boom! you're done!
