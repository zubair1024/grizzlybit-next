...
Firstly, we will begin with switching over to our postgres user
...
Next, we will crate a new database named gis with the owner as the new user we have just created
...
Since this process will take quite a bit of time. It is recommended to run the entire process using a `screen` multiplex session. For a quickie on `screen` please [click here](https://grizzlybit.info/blog/how-to-use-screen-on-linux-to-detach-and-reattach-your-terminal). 👈
...
Shapefiles is a format in which we can store GeoJSON data.
